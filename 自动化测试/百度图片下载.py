...
# 请求地址
driver.get('http://image.baidu.com/search/index?tn=baiduimage&ps=1&ct=201326592&lm=-1&cl=2&nc=1&ie=utf-8&word=' + keyword)
# 浏览图片
print('正在浏览图片请稍等……')
scroll_y = 0
page = 0
while page < count // 10:
    ...
    page += 1
    img_count = len(driver.find_elements_by_class_name('imgitem'))
    if img_count > count:
        break
    print(str(page) + '次')
    print(str(img_count) + '张')
    time.sleep(1)
print(len(driver.find_elements_by_class_name('imgitem')))
# 图片url列表
img_url_list = [item.get_attribute('data-objurl') for item in driver.find_elements_by_class_name('imgitem')]
driver.close()
# 创建文件夹
filename += '_baidu'
